...
def create_scheduler(survey_length=3652.5, maxDither=0.7, moon_illum_limit=40, nexp=2,
                     rolling_nslice=2, rolling_strength=0.9, gsw=3.0, ddf_season_frac=0.2,
                     nside=32):
    """Generate a baseline Core_scheduler object
    """
    ...
    footprints = make_rolling_footprints(fp_hp=footprints_hp, mjd_start=conditions.mjd_start,
                                         sun_RA_start=conditions.sun_RA_start, nslice=rolling_nslice, scale=rolling_strength,
                                         nside=nside, wfd_indx=wfd_indx)

    # Set up the DDF surveys to dither
    u_detailer = detailers.Filter_nexp(filtername='u', nexp=1)
    dither_detailer = detailers.Dither_detailer(per_night=per_night, max_dither=maxDither)
